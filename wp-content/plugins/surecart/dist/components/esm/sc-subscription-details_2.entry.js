...
const ScSubscriptionDetails = class {
  ...
  renderRenewalText() {
    ...
    if (this.hasPendingUpdate) {
      ...
      return (h("span", { "aria-label": wp.i18n.sprintf(wp.i18n.__('Renewal Update - Your plan switches to %1s on %2s', 'surecart'), this.pendingPrice.product.name, formatTime(this.subscription.current_period_end_at)) }, wp.i18n.__('Your plan switches to', 'surecart'), " ", h("strong", null, this.pendingPrice.product.name), " ", wp.i18n.__('on', 'surecart'), ' ', h("sc-format-date", { date: this.subscription.current_period_end_at, type: "timestamp", month: "long", day: "numeric", year: "numeric" })));
    }
  ...
  showWarning() {
    ...
    // no payment method.
    if ((_a = this.subscription) === null || _a === void 0 ? void 0 : _a.payment_method) {
      return false;
    }
  ...
  render() {
    return (h("div", { class: "subscription-details" }, this.hasPendingUpdate && (h("div", null, h("sc-tag", { size: "small", type: "warning" }, wp.i18n.__('Update Scheduled', 'surecart')))), h("sc-flex", { alignItems: "center", justifyContent: "flex-start" }, h("sc-text", { "aria-label": wp.i18n.sprintf(wp.i18n.__('Plan name - %s', 'surecart'), this.renderName()), style: { '--font-weight': 'var(--sc-font-weight-bold)' } }, this.renderName()), this.renderActivations()), !this.hideRenewalText && h("div", null, this.renderRenewalText(), " "), h("slot", null), h("sc-dialog", { label: wp.i18n.__('Activations', 'surecart'), onScRequestClose: () => (this.activationsModal = false), open: !!this.activationsModal }, h("sc-card", { "no-padding": true, style: { '--overflow': 'hidden' } }, h("sc-stacked-list", null, (this.getActivations() || []).map(activation => {
    ...
    })))), this.showWarning() && (h("div", null, h("sc-tag", { type: "warning" }, h("div", { class: "subscription-details__missing-method" }, h("sc-icon", { name: "alert-triangle" }), wp.i18n.__('Payment Method Missing', 'surecart')))))));
  }
...
const ScSubscriptionStatusBadge = class {
  ...
  render() {
    return (h("sc-tag", { "aria-label": wp.i18n.sprintf(wp.i18n.__('Plan Status - %s', 'surecart'), this.getText()), type: this.getType() }, this.getText()));
  }
};
