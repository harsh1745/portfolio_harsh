...
import { a as apiFetch, h as handleNonceError } from './fetch-2525e763.js';
...
const ScProductItemList = class {
  constructor(hostRef) {
    ...
    this.scSearched = createEvent(this, "scSearched", 7);
    this.scProductsViewed = createEvent(this, "scProductsViewed", 7);
    this.ids = undefined;
  ...
  async fetchProducts() {
    ...
    try {
      ...
      if (!!collectionIds.length || !!this.query) {
        speak(wp.i18n.sprintf(wp.i18n.__('%s products found', 'surecart'), this.pagination.total));
      }
    }
    catch (response) {
  ...
  render() {
    ...
    return (h("div", { class: {
      ...
      } }, this.error && (h("sc-alert", { type: "danger", open: true }, this.error)), (this.searchEnabled || this.sortEnabled || this.collectionEnabled) && (h("div", { class: "product-item-list__header" }, h("div", { class: "product-item-list__controls" }, h("div", { class: "product-item-list__sort" }, this.sortEnabled && (h("sc-dropdown", { style: { '--panel-width': '15em' } }, h("sc-button", { type: "text", caret: true, slot: "trigger" }, h("sc-visually-hidden", null, wp.i18n.__('Dropdown to sort products.', 'surecart'), " "), this.renderSortName(), h("sc-visually-hidden", null, " ", wp.i18n.__('selected.', 'surecart'))), h("sc-menu", { "aria-label": wp.i18n.__('Sort Products', 'surecart') }, h("sc-menu-item", { "aria-label": wp.i18n.__('Sort by latest', 'surecart'), onClick: () => (this.sort = 'created_at:desc') }, wp.i18n.__('Latest', 'surecart')), h("sc-menu-item", { "aria-label": wp.i18n.__('Sort by oldest', 'surecart'), onClick: () => (this.sort = 'created_at:asc') }, wp.i18n.__('Oldest', 'surecart')), h("sc-menu-item", { "aria-label": wp.i18n.__('Sort by name, A to Z', 'surecart'), onClick: () => (this.sort = 'name:asc') }, wp.i18n.__('Alphabetical, A-Z', 'surecart')), h("sc-menu-item", { "aria-label": wp.i18n.__('Sort by name, Z to A', 'surecart'), onClick: () => (this.sort = 'name:desc') }, wp.i18n.__('Alphabetical, Z-A', 'surecart'))))), this.collectionEnabled && ((_a = this.collections) !== null && _a !== void 0 ? _a : []).length > 0 && (h("sc-dropdown", { style: { '--panel-width': '15rem' } }, h("sc-button", { type: "text", caret: true, slot: "trigger" }, h("sc-visually-hidden", null, wp.i18n.sprintf(wp.i18n.__('Dropdown to filter products by collection. %s selected.', 'surecart'), ((_b = this.selectedCollections) === null || _b === void 0 ? void 0 : _b.length) ? this.selectedCollections.map(collection => collection === null || collection === void 0 ? void 0 : collection.name).join(',') : wp.i18n.__('None', 'surecart'))), h("span", { "aria-hidden": true }, " ", wp.i18n.__('Filter', 'surecart'))), h("sc-menu", { "aria-label": wp.i18n.__('Filter products', 'surecart') }, ((_c = this.collections) !== null && _c !== void 0 ? _c : []).map(collection => {
      return (h("sc-menu-item", { checked: this.selectedCollections.some(selected => (selected === null || selected === void 0 ? void 0 : selected.id) === (collection === null || collection === void 0 ? void 0 : collection.id)), onClick: () => this.toggleSelectCollection(collection), key: collection === null || collection === void 0 ? void 0 : collection.id, "aria-label": wp.i18n.sprintf(wp.i18n.__('Filter by %s', 'surecart'), collection === null || collection === void 0 ? void 0 : collection.name) }, collection.name));
    }))))), h("div", { class: "product-item-list__search" }, this.searchEnabled &&
      (((_d = this.query) === null || _d === void 0 ? void 0 : _d.length) && this.query === this.currentQuery ? (h("div", { class: "product-item-list__search-tag" }, h("div", { class: "product-item-list__search-label" }, wp.i18n.__('Search Results:', 'surecart')), h("sc-tag", { clearable: true, onScClear: () => {
          this.query = '';
          this.currentQuery = '';
          this.updateProducts();
        ...
        }, value: this.query, clearable: true }, this.query ? (h("sc-icon", { class: "clear-button", slot: "prefix", name: "x", onClick: () => {
          this.query = '';
        } })) : (h("sc-icon", { slot: "prefix", name: "search" })), h("sc-button", { class: "search-button", type: "link", slot: "suffix", busy: this.busy, onClick: () => {
          this.updateProducts(true);
        } }, wp.i18n.__('Search', 'surecart'))))))), this.collectionEnabled && this.selectedCollections.length > 0 && (h("div", { class: "product-item-list__search-tag" }, this.selectedCollections.map(collection => (h("sc-tag", { key: collection === null || collection === void 0 ? void 0 : collection.id, clearable: true, onScClear: () => {
        this.toggleSelectCollection(collection);
      } }, collection === null || collection === void 0 ? void 0 : collection.name))))))), !((_e = this.products) === null || _e === void 0 ? void 0 : _e.length) && !this.loading && (h("sc-empty", { class: "product-item-list__empty", icon: "shopping-bag" }, wp.i18n.__('No products found.', 'surecart'))), h("section", { class: "product-item-list", "aria-label": wp.i18n.__('Product list', 'surecart') }, this.loading
      ? [...Array(((_f = this.products) === null || _f === void 0 ? void 0 : _f.length) || this.limit || 10)].map((_, index) => (h("div", { class: "product-item-list__loader", key: index }, (this.layoutConfig || []).map(layout => {
        var _a, _b;
      ...
      : (this.products || []).map((product, index) => {
        return (h("sc-product-item", { key: product === null || product === void 0 ? void 0 : product.id, ...(this.products.length - 1 === index
            ? {
      ...
      })), this.isPaginationAvailable() && (h("div", { class: {
      ...
      } }, h("sc-pagination", { page: this.page, perPage: this.limit, total: this.pagination.total, totalPages: this.pagination.total_pages, totalShowing: this.limit, onScNextPage: () => this.doPagination(this.page + 1), onScPrevPage: () => this.doPagination(this.page - 1) }))), (this.busy || this.loading) && h("sc-block-ui", null)));
  }
